...
manipulation tools to create images of different sizes and bit depths. You will
then arrange these images into gallery with a tabular format that is styled by
...
and using software tools that supported the implementation, testing, and
debugging of a web site.
...
\begin{figure}[t]
  ...
  \caption{{\bf A Conceptual Representation of the Image Gallery}. In this
    ...
    means that the gallery should give the size of the image in kilobytes
  (KB).}~\label{fig:wireframe}
...
the image) and its file size in kilobytes. To discover an image's size in
kilobytes, you can type the command \program{ls -lh} in the terminal window and
...
Please notice that the second and fourth rows of the image gallery should
feature images that are not resized using the \program{convert} program.
Instead, you should learn how to resize images using HTML and/or CSS source
code. Critically, you should think about the size in kilobytes of these
``resized'' images and consider the trade-offs associated with adopting one of
these two different mechanisms for image resizing. Please don't forget that the
second and fourth rows should still display, in a textual format, the details
about the resolution and file size for these images. Finally, students should
remember to check the correctness of their HTML by running the command
\command{htmlhint src/www/index.html} in their terminal window. If the image
gallery in your web site is incorrect, then please discuss the problems in your
HTML or CSS with a teaching assistant or the instructor.
